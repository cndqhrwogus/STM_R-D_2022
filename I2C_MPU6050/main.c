...
float accel_x, accel_y, accel_z; //�??��?�� ?��?��?�� 최종?��?�� 보정�?!!!
...
/* Configure the peripherals common clocks */
  ...
  while (1)
  {
	  mpu6050_get(&ax,&ay,&az,&gx,&gy,&gz,&tmpr);

	  las_angle_gx = pitch;	//최근�? ?��?��
	  las_angle_gy = roll;
	  las_angle_gz = yaw;
	  ...
	  accel_x = a_x - bas_a_x; // 가속도(직선) X축에 대한 현재 값 - 가속도 센서의 평균값
	  accel_y = a_y - bas_a_y;
	  accel_z = a_z + (16384 - bas_a_z);

	  angle_ax = atan(accel_y/sqrt(pow(accel_x,2) + pow(accel_z,2)))*(180/3.141592);
	  angle_ay = atan(-1.000*accel_x/sqrt(pow(accel_y,2) + pow(accel_z,2)))*(180/3.141592);
	  ...
	  //상보 필터로 각도 측정
	  alpha = 0.96;
	  pitch = alpha*angle_gx + (1.000 - alpha)*angle_ax;
	  roll = alpha*angle_gy + (1.000 - alpha)*angle_ay;
	  yaw = angle_gz;
